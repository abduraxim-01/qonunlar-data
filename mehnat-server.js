...
// Faqat Mehnat kodeksiga ulanadigan alohida DB Pool
const pool = new pg.Pool({
  connectionString: process.env.DATABASE_URL,
});

const mcpServer = new Server({
  name: "mehnat-kodeksi-api",
  version: "1.0.0"
}, {
  capabilities: { tools: {} }
});
...
// 1. Tool'larni ro'yxatdan o'tkazish (Faqat mehnatga oid bittagina yengil tool)
...
mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name !== "mehnat_kodeksi_qidiruv") {
...
app.get("/api/sse", async (req, res) => {
  const sessionId = req.query.sessionId || "mehnat-session";
...
app.listen(PORT, () => {
  console.log(`Mehnat MCP Server ${PORT}-portda ishlamoqda`);
});